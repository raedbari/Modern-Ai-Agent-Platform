...
def test_session_family_id_is_optional() -> None:
    """Verify that tokens can be created without session_family_id."""
    settings = _settings()
    user_id = str(uuid.uuid4())
    tenant_id = str(uuid.uuid4())
...
def test_jti_is_unique_per_token() -> None:
    ...
    tokens = [
        create_access_token(user_id, tenant_id, settings)
        for _ in range(20)
    ]
...
def test_invalid_signature_is_rejected() -> None:
    ...
    token = create_access_token(user_id, tenant_id, settings)
...
def test_non_revoked_token_is_still_valid() -> None:
    ...
    token_a = create_access_token(user_a_id, tenant_id, settings)
    token_b = create_access_token(user_b_id, tenant_id, settings)
...
def test_decode_token_raises_when_secret_is_not_configured() -> None:
    ...
    user_id = str(uuid.uuid4())
    tenant_id = str(uuid.uuid4())
    token = create_access_token(user_id, tenant_id, settings_with)
...
def test_empty_tenant_id_is_rejected() -> None:
    """Verify that tokens with empty tenant_id claim are rejected."""
    now = datetime.now(timezone.utc)
    payload = {
        "sub": str(uuid.uuid4()),
    ...
    with pytest.raises(TenantTokenError, match="tenant_id"):
        decode_access_token(token, settings)
